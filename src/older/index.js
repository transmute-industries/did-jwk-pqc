...
const generateKeyPair = async (alg, options = {extractable: true}) => {
  const {publicKey, privateKey} = await jose.generateKeyPair(alg, options);
  ...
  if (options.extractable) {
    ...
    kp.privateKeyJwk = formatJwk({...privateKeyJwk, alg, kid});
  } else {
...
const decryptWithKey = async (jwe, privateKey) => {
  privateKey = privateKey.kty ? await jose.importJWK(privateKey) : privateKey;
...
const sign = async (payload, privateKey, header = {}) => {
  privateKey = privateKey.kty ? await jose.importJWK(privateKey) : privateKey;
